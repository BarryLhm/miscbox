...
class ArgParser {
	...
	std::vector<std::unordered_map<std::string, uint>> scanlist_;
	...
	[[noreturn]] void invalid_arg_(InvalidArg reason, uint pos);
	...
	ArgParser& scanlist_add(uint pos, const std::string& arg, uint num);
