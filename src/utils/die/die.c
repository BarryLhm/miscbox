...
{
	if (argc <= 1)
		errexit(EXIT_ERROR, E_INVALID_ARG, "die type not specified\n");

	deathtype_t type = str2enum(die_types, die_typec, argv[1]);
	switch (type) {
	case EN_NOT_FOUND:
		errexit(EXIT_ERROR, E_INVALID_ARG, "%s: unknown way to die\n", argv[1]);
	case D_NULLPTR: {
		if (argc > 2)
			errexit(EXIT_ERROR, E_INVALID_ARG, "extra argument\n");
	...
	case D_RAISE: {
		...
		if (argv[2][0] == '\0')
			errexit(EXIT_ERROR, E_INVALID_ARG, "empty string\n");
		...
		if (signal) {
			if (signal < 0 || signal > 64)
				errexit(EXIT_ERROR, E_INVALID_ARG, "sigid must be 1~64\n");
			if (raise(signal))
				errexit(EXIT_ERROR, E_FAILED, "raise failed\n");
			errexit(EXIT_ERROR, E_FAILED, "I'm not killed!\n");
		}
		if (errno == ERANGE)
			errexit(EXIT_ERROR, E_INVALID_ARG, "number overflow\n");
		if (endptr[0] == '\0')
			errexit(EXIT_ERROR, E_INVALID_ARG, "there's no signal 0\n");
