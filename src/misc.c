...
int strcase2enum_cmp(const void* key, const void* elem)
{
	return strcasecmp((const char*)key, ((const str2enum_t*)elem)->name);
}
...
{
	str2enum_t* item = bsearch(str, list, size, sizeof(str2enum_t), strcase2enum_cmp);
	if (!item) return EN_NOT_FOUND;
	return item->val;
}
